...
def download_candidate(
    ...
    # bounded_dest keeps the ABSOLUTE path inside the platform limit; safe_filename only
    # bounds the filename, which is not enough under a deep output dir on Windows.
    dest = bounded_dest(doc_dir, stem, ext)
    base_stem = dest.stem

    counter = 1
    while dest.exists():
        dest = bounded_dest(doc_dir, f"{base_stem}_{counter}", ext)
        counter += 1
