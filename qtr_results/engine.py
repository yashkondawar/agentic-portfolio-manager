...
def run(params: Optional[Dict[str, Any]] = None, price_fn: Optional[Callable] = None) -> Dict[str, Any]:
    ...
    for analysis in strong:
        ...
        if not entry_price or entry_price <= 0:
            ...
            continue
        plan = build_target_plan(analysis, entry_price)
        if plan is None:
            continue
...
def _render_report(**kw) -> str:
    ...
    lines += ["", "## New buys", ""]
    if kw["new_picks"]:
        lines.append("| Symbol | Entry | Target % | Target Rs | Trail stop % | Method | Strength | Source | Rationale |")
        lines.append("|---|---|---|---|---|---|---|---|---|")
        for p in kw["new_picks"]:
            src = ", ".join(p.get("sources", [])) or "-"
            lines.append(
                f"| {p['symbol']} | {fmt_price(p['entry_price'])} | {p['target_pct']:.1f}% | "
                f"{fmt_price(p['target_price'])} | {p['trailing_stop_pct']:.1f}% | {p['method']} | "
                f"{p['strength_score']:.0f} | {src} | {p['rationale']} |"
            )
    else:
